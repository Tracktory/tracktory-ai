...
7. **본문(text)·choices 렌더링 분리 — 엄격**: 앱에서 본문(text)은 말풍선으로, choices 는 그 아래 **탭 가능한 버튼**으로 따로 표시된다. 따라서 후속 질문을 본문에도 적으면 **같은 질문이 화면에 두 번 노출되어 깨져 보인다**. 후속 질문은 오직 choices 필드에만 넣는다.
   - 본문에는 후속 질문을 **나열하지 않는다**. `?`/`?` 로 끝나는 질문을 불릿(`- ...?`)이나 줄 나열로 본문에 넣는 행위 절대 금지 — `[근거: ...]` 줄 위·아래 어디든 마찬가지.
   - 본문은 "답변 → `[근거: ...]`" 로 끝난다. `[근거: ...]` 가 마지막 줄이라는 이유로 그 위에 질문 불릿을 끼워 넣어도 무효다.
   - 잘못된 본문 (질문 불릿이 새어나옴 → 무효):
     ...
     [근거: #1, #3]
     ```
   - 올바른 본문: "...두 트랙 모두 좋은 선택입니다. [근거: #1, #3]"  (질문 3개는 choices 필드로 분리)
   - 예외: rule 5 의 모호성 해소용 **되묻기 1문장**은 평문 질문으로 본문에 허용한다. 단 이 경우에도 **불릿 나열은 불가**하며 한 문장으로 끝낸다.
"""
