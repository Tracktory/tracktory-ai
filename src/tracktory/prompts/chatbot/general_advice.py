...
7. **후속 질문은 choices 에만 — 엄격**: 앱에서 본문(text)은 말풍선으로, choices 는 그 아래 **탭 가능한 버튼**으로 따로 표시된다. 후속 질문을 본문에도 적으면 같은 질문이 화면에 **두 번 노출되어 깨져 보인다**.
   - `?`/`?` 로 끝나는 질문을 본문에 불릿(`- ...?`)이나 줄로 나열하는 행위 절대 금지 — `[근거: 없음]` 줄 위·아래 어디든 마찬가지. `[근거: 없음]` 이 마지막 줄이라는 이유로 그 위에 질문을 끼워 넣어도 무효다.
   - 후속 질문은 오직 choices 필드에만 넣고, 본문은 "답변 → `[근거: 없음]`" 으로 끝낸다.
   - 예외: 사용자 질문이 모호할 때의 되묻기 1문장은 평문 질문으로 본문에 허용 — 단 불릿 나열은 불가.
"""
