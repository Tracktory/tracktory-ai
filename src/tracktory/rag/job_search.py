...
class JobSearchClient(Protocol):
    ...
    def rag_search_jobs(self, query: str, top_k: int = 3) -> list[RagSearchResult]:
        ...
        구현체는 외부 검색 실패 시 ``RagSearchError`` 를 raise 해야 한다.
        직무 매칭 노드는 이 예외를 catch 하여 카테고리 사전 매핑 fallback
        분기로 전환한다.
        """
        ...
...
class RagSearchError(Exception):
    ...
    구현체에서 디버깅 컨텍스트 (요청 페이로드 키 / 응답 상태 코드 등) 를
    실어 보내되, 개인정보·자격증명은 포함하지 않는다.
    """
