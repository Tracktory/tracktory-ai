...
class JobCandidate(BaseModel):
    ...
    분모로 흐른다. ``similarity`` 와 ``match_score`` 는 항상 같은 값으로
    채워지며 (정상 경로의 검색 점수, fallback 시 0.0), 정상 경로에서는 이수
    과목 부스팅이 후처리로 두 필드에 동일하게 가산될 수 있다. 두 필드 동시
    보존은 다운스트림 노드가 어느 키를 참조해도 동일하게 동작하도록 보장한다.
    ...
    Attributes:
        ...
        match_score: 사용자-직무 적합도 ([0, 1]). 정상 경로에서는 검색 점수에
            이수 과목 부스팅이 가산된 값. 다운스트림 트랙 시너지 노드와의
            호환을 유지한다.
        similarity: 직무 매칭 노드가 채우는 점수 (정상 경로의 검색 점수 +
            이수 과목 부스팅, fallback 시 0.0) ([0, 1]).
