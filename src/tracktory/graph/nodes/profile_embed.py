"""정규화된 사용자 프로필을 결정론적 키워드 나열로 직렬화한다.

LLM 직렬화는 동일 입력에서도 출력이 달라질 수 있어 파이프라인 비교 실험의
통제 조건을 깨뜨린다. 그 대신 ``profile_embed_template.yaml`` 의 결정론적
패턴으로 키워드 나열을 합성한다.

직무 검색 인덱스(채용공고)의 어휘는 키워드 중심이라, 관형구로 풀어쓴 문장형
질의는 질의-문서 유사도가 낮게 잡힌다. 같은 입력을 키워드 나열로 직렬화하면
인덱스 어휘와의 표면 정합이 올라가 유사도가 높아진다 (질의 신호만 바꾸는
것이며 인덱스 측 임베딩은 불변).

본 노드는 더 이상 자체 임베딩 호출을 수행하지 않는다. 직무 검색 boundary
(``JobSearchClient.rag_search_jobs``) 가 질의 텍스트를 받아 임베딩·검색·
재정렬을 일괄 처리하므로, 자체 코드는 직렬화한 키워드 질의만 다음 단계로
넘긴다. ``completed_courses`` 는 이수 과목 집합 필터로만 사용되며 의미
직렬화에는 포함하지 않는다.
...
def _serialize_profile(profile: dict[str, Any], template: dict[str, Any]) -> str:
    """카테고리형 프로필을 결정론적 키워드 나열로 변환한다.

    템플릿의 절(clause) 정의를 선언 순서대로 순회하며, 값이 비어 있지 않은
    필드만 키워드로 만들어 이어 붙인다. 선택 입력(취업 가치·선호 회사 유형·
    공부해본 분야)이 누락돼도 빈 구분자 없이 질의가 완결되도록 빈 필드의 절은
    건너뛴다.
    ...
    LLM 미사용이므로 동일 입력은 항상 동일 출력을 생성한다 (절 순서·값 순서·
    구분자가 모두 결정론적).
    """
